...
1. Create a workspace folder anywhere
...
pip install path/to/fluxcard_repo
```
4. Create the Clock file in the location of your choice, it must be accessible by all devices you want to clock in and out with. This does not have to be in your working folder, just needs to have the location set in the config file.
5. Start the file off by putting `==` in the top line and make sure there is no newline after it (plenty of code editors will add a newline automatically, make sure this doesn't happen)
6. Create `config.toml` in your working folder and set your default parameters here, such as the location of the Clock file with respect to the config file or an absolute path and output timezone.
7. Set up the Apple Shortcuts. On the devices which you plan to clock in and out, import the shortcuts Clock In and Clock Out. See the README in the shortcuts folder for more information
...
8. Run the program using one of the following. Remember that the program looks for the config file in the current working directory, unless you specify a different config file on the command line.
```sh
fluxcard
flux
```
